...
let reducer = 
  let redux state = function
    ...
    | `Dec -> state -. 1.0
  in
  let jsonify = Js.Json.number in
  Bs_redux.Reducer.make redux jsonify
...
let _ =
  suite "bs-redux reducer" [
    ...
    Sync ("can convert value to json", fun () -> let v = Js.Json.number 1.0 in assert_eq v (Bs_redux.Reducer.jsonify reducer 1.0));
  ];
